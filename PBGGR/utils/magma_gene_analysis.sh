...
sort -gk9 $magma_file | awk '{print $1 "\t" NR "\t" $9}' | fgrep -wf top.unique_magma - | sort -k1 > aux1
sort -gk3 $bf_file | awk '{print $1 "\t" NR "\t" $3}' | fgrep -wf top.unique_magma - | sort -k1 > aux2  # warning fgrep add every prefix match
tail -n +2 $exome_file | sort -gk2 | awk '{print $1 "\t" NR "\t" $2}' | fgrep -wf top.unique_magma - | sort -k1 > aux3
awk '{print $1 "\t" $2 "\t" $3 "\t" $4}' $gene_file | fgrep -wf top.unique_magma - | sort -k1 > aux4
