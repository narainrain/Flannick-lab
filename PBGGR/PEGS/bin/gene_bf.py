...
def BGAM(options):
    ...
    if options.simulate:
        ...
        gene_to_log_bf, component_to_snp, gene_extend_to_chrom_start_stop, snp_to_chr_pos, snp_to_z, snp_to_n, \
        ld_dict, snp_to_se, snp_to_beta = get_data()
    # ------------------------------------------------------------------------------------------------------------------
    ...
    for component in component_to_snp:
        ...
        snp_zs, snp_ns, component_snps_obs, has_z, snp_betas, snp_ses = \
            extract_snps(snp_to_index, snp_to_z, snp_to_n, snp_to_beta, snp_to_se)
        ...
        # likelihood calculation of the genes in the component (Greedy, Power set, Single gene) ------------------------
        gene_to_alt, gene_to_null = calculate_likelihood(genes_in_component, component_snps_obs, gene_to_snp_probs,
                                                         cor_matrix_obs, snp_zs, gene_to_log_bf)

        # Calculation of bayes factor for each gene in the component  --------------------------------------------------
        bf_calculation(gene_to_log_bf, gene_to_null_h, gene_to_alt_h, gene_to_alt_num_snps, gene_to_null_num_snps,
